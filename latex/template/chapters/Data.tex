\chapter{Data}
\textcolor{red}{Idas far kan lige skrive her om fjernvarmesystemet.} \textcolor{red}{tilføj hvordan fjernvarmesystemet er opbygget og hvordan målingerne er foretaget.}
når vandet kommer ind i huset, sidder der en måler, som måler Temperaturen på det der kommer ind og det der kommer ud igen samt mængden af det. \\
...
\begin{equation}
    ...
    \label{eq: Q_heat}
\end{equation}
where $c$ is the specific heat capacity for water which is $4.186 \ kJ/kg\cdot C^{\circ}$, $m$ is the water density in $kg$ and is calculated from the volume as $1m^3 = 1000 \ kg$ and $\Delta T$ is the cooling degree in $C^{\circ}$. In this way, the heat consumption is converted from $kJ$ to $kwh$ (since $1 \ kwh = 3600 \ kJ$) which is the desired unit for the heat consumption.
\begin{table}[H]
    \centering
    \begin{tabular}{ll}
     \hline
