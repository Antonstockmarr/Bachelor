\chapter{Data}
%\textcolor{red}{Idas far kan lige skrive her om fjernvarmesystemet.} \textcolor{red}{tilføj hvordan fjernvarmesystemet er opbygget og hvordan målingerne er foretaget.} når vandet kommer ind i huset, sidder der en måler, som måler Temperaturen på det der kommer ind og det der kommer ud igen samt mængden af det. \\
...
\noindent A physical factor that could possibly affect the heat consumption is the sun. In raw data, the attributes \texttt{Condition}, \texttt{SunHour},  and \texttt{UltraVioletIndex} can be seen as explanatory variables for the sun. Instead, an attribute, \texttt{Radiation}, is added to calculate the solar radiation for a given day. This attribute is determined with use of the \texttt{R} function \texttt{calcSol} from the library \texttt{solaR}. This value is the radiation of the sun if there is no cloud coverage. That is multiplied by the \texttt{SunHour} to weight the radiation with the amount of sun actually penetrating the clouds. The ultraviolet index is a measurement of the strength of ultraviolet radiation and since the attribute \texttt{Radiation} is more exact, \texttt{UltravioletIndex} is removed from the weather data set. \texttt{SunHour} is also removed, since it is now included in \texttt{Radiation}.
