...
\noindent In addition, the models are used to predict 31 days ahead in order to verify that the models capture the overall behaviour of data. It must be known that the model data has been divided into training and test sets in order to make predictions. The test set consists of the last 31 days and the remaining observations constitute the training set. The predictions are made using the function \texttt{predict} in \texttt{R}. \cref{fig: weather_pred} shows the input values used for the predictions. It is expected that 
\begin{figure}
    \centering
...
\end{figure}
In consistency with the other results, it is the two houses, 18 and 55, whose predictions are illustrated in \cref{fig: lmpred_55L} and \cref{fig: lmpred_18L}. 
\begin{figure}
    \centering
