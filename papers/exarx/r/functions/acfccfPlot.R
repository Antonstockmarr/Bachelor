acfccfPlot <- function(fit,X)
{
    ...
    ccf(eps,X[,"Te.l0"],lag.max=100,xlim=c(1,50),ylab="CCF",main="CCF(eps,Te)",ylim=c(-0.5,0.5))
    ## The cross-correlation function between the residuals and the vertical global radiation
    ccf(eps,X[,"Gv.l0"],lag.max=100,xlim=c(1,50),ylab="CCF",main="CCF(eps,Gv)",ylim=c(-0.5,0.5))
}
